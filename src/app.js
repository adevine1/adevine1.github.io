...
const diceArray = ["d1", "d2", "d3", "d4", "d5"];
...
function loadStuff() {
  ...
  else document.getElementById("effects").innerText = "effects off";
}
...
function showHighScores() {
  ...
  for (var key in loadScores) {
    ...
    let spaces = "";
    for (let x = 0; x < 20 - nameLength.length; x++) spaces += " ";
    highScoreString +=
      "\n" +
      loadScores[key][1] +
      " scored " +
  ...
  if (highScoreString === "SCORE HISTORY: \n")
    highScoreString =
      "Your footsteps echo through an empty Hall of Records, inspiring you to action.";
...
function submitValue(element) {
  ...
  if (a !== "") {
    ...
    for (let x = 0; x < 5; x++) {
      document.getElementById(diceArray[x]).style.color = "white";
...
function rollTheDice() {
  ...
  if (document.getElementById("rollerbutton").innerText !== "Roll") {
    ...
    if (result !== null) {
      ...
      let month = entireDate.getMonth ();
      let day = entireDate.getDate ();
      let year = entireDate.getFullYear ();
      let dateString = month + "-" + day + "-" + year;

      newScore[0] = endscore;
      newScore[1] = result.toUpperCase();
      newScore[2] = dateString;
